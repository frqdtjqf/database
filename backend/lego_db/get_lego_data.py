...
def get_lego_parts(data : list[dict[str, str]], version_name: str) -> list[LegoPart]:
...
def get_template_minifigures(data : list[dict[str, str]], version_name: str) -> list[TemplateMinifigure]:
    version = get_csv_part_versions(version_name)
    minifigures = []
    for row in data:
        id = row.get(version.id)
